...
IHS4_num <- function(nodules, abscesses, draining_tunnels) {
  ...
  checkmate::assert_integerish(nodules, lower = 0, any.missing = FALSE, null.ok = FALSE)
  checkmate::assert_integerish(abscesses, lower = 0, len = length(nodules), any.missing = FALSE, null.ok = FALSE)
  checkmate::assert_integerish(draining_tunnels, lower = 0, len = length(nodules), any.missing = FALSE, null.ok = FALSE)
